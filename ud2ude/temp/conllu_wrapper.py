...
def serialize_spacy_doc(orig, converted):
    from spacy.tokens import Doc
    from spacy import attrs
    import numpy as np
    
    attrs = list(attrs.NAMES)
    attrs.remove('SENT_START')
    orig_attrs = orig.to_array(attrs)
    new_attrs = np.append(orig_attrs, [orig_attrs[round(iid)] for iid, tok in converted.items() if round(iid) != iid], axis=0)
    
    new_doc = Doc(orig.vocab, words=[t.get_conllu_field("form") for iid, t in converted.items() if iid != 0])
    new_doc.from_array(attrs, new_attrs)
    
    # TODO - set attributes of all tokens
    # TODO - set whitespace of last token and all new tokens. see spacy.tokens.TOKEN.whitespace_
